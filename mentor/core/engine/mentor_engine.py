...
class MentorEngine:
    def __init__(self):
        ...
        # Load mentor roles and instructions
    ...
    async def generate_intro_and_topics(
        ...
        print(f"Prompt for generate_intro_and_topics: {prompt_content}")
    ...
    async def generate_topic_prompts(
        ...
        messages = [{"role": "user", "content": prompt_content}]
        try:
            llm_response = await self._get_llm_completion(messages, temperature=0.5, max_tokens=500)
